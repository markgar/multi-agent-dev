...
VALIDATOR_JOURNEY_SECTION = (
    ...
    "JOURNEYS TO RUN:\n"
    "{journey_list}\n"
    "For any journey failure, create a `bug-<timestamp>.md` in `bugs/` with the "
    "journey ID, the failing step, expected vs actual behavior, and which milestone.\n"
)
...
VALIDATOR_LEGACY_SCOPE = (
    ...
    "create a `bug-<timestamp>.md` in `bugs/` with a [missing-requirement] prefix "
    "describing what was required and what is missing.\n\n"
    "C. FIXED BUG VERIFICATION:\n"
    "- List `bugs/` for `bug-*.md` files that have a matching `fixed-*.md` file.\n"
    "- Read each original bug description.\n"
    "- Verify the fix is working in the running app.\n"
    "- If the issue persists, create a new `bug-<timestamp>.md` describing the "
    "regression. Do NOT remove the `fixed-*.md` — just file a new bug.\n\n"
)

VALIDATOR_LEGACY_RESULTS_TAGS = (
    "  [A] for current milestone validation tests\n"
    "  [B] for requirements coverage checks\n"
    "  [C] for fixed bug re-verification\n"
    "- Example lines:\n"
    "  PASS  docker compose build -> success\n"
    "  PASS  container startup -> healthy after 2s\n"
    ...
    "  PASS  [C] bug-20260220-143012 (500 on empty POST) -> now returns 400\n"
    "  FAIL  [C] bug-20260221-091500 (duplicate titles allowed) -> regression, "
