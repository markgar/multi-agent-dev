...
PLANNER_INITIAL_PROMPT = (
    ...
    "BACKLOG.md. Do NOT create milestone files — a separate milestone planner handles "
    "that after the backlog passes quality checks.\n\n"
    ...
    "session. SMALLER MILESTONES ARE BETTER — they reduce context drift, catch "
    "issues earlier through more frequent review/test/validate cycles, and provide "
    "more recovery checkpoints. Target 3-5 tasks per story. This is the critical "
    "sizing constraint — too few tasks means the session overhead (git claim, "
    "planner call, context loading) isn't justified; too many and the LLM loses "
    "coherence.\n"
    "- If a feature's full vertical slice (backend + frontend) fits in 5 tasks, "
    "keep it as ONE story. Only split backend/frontend into separate stories when "
    "either half alone would need 4+ tasks.\n"
    ...
    "- Do NOT create stories that would produce more than 7 tasks. Split those at a "
    "natural boundary.\n"
    "- Example: 'Venues backend API + admin pages' is ONE story if it's 5 tasks "
    "(entity + repo, service, controller, list page, detail page). Do NOT split "
    "that into 'Venues — backend' (2 tasks) and 'Venues — admin pages' (3 tasks).\n"
    "- Example: 'Members backend API + admin pages + profile page + assignment UI' "
    "is TOO BIG at 8+ tasks — split into 'Members — backend & admin' and "
    ...
    "CONTAINERIZATION: Do not create stories for Dockerfiles, docker-compose "
    "files, or deployment configuration. A separate validator agent handles that. "
    "Do NOT add these to BACKLOG.md.\n\n"
    "TESTING: Do not create stories for writing tests. A separate testing "
    "agent writes tests automatically after each milestone. Do NOT add test stories "
    "to BACKLOG.md — they will never be planned into milestones and create "
    "false-positive warnings.\n\n"
    "Commit BACKLOG.md with message '[planner] Create backlog'. Run git pull "
    "--rebase, and push."
)
...
PLANNER_PROMPT = (
    ...
    "- SMALLER MILESTONES ARE BETTER. Target 3-5 tasks. Under 3 suggests tasks "
    "might be too coarse. Over 7 suggests a natural split point exists — look for "
    "it.\n"
    "- Smaller milestones = shorter sessions = less context drift = more frequent "
    "review/test/validate cycles = earlier bug detection.\n"
    "- Do NOT pack multiple concerns into one task to keep the milestone small.\n"
    "- If a feature area needs more than 5 tasks, prefer splitting into sequential "
    "milestones (e.g. 'Members — backend' then 'Members — frontend').\n\n"
    "EXAMPLE of a well-sized milestone — 'Members backend':\n"
    "- Create Member entity (Id, FirstName, LastName, Email, Role enum, IsActive, "
    "OrganizationId) and MemberRepository implementing BaseRepository<Member>\n"
    ...
    "This is 3 tasks — backend for one feature. The frontend gets its own milestone.\n\n"
...
BACKLOG_QUALITY_PROMPT = (
    ...
    "Each story becomes one Copilot session. Stories should be sized to produce 3-5 "
    "tasks each. Smaller milestones are better — they reduce context drift and "
    "enable more frequent review/test/validate cycles. Check for BOTH problems:\n"
    ...
    "If two adjacent stories for the same feature would each produce fewer than 3 "
    "tasks, merge them into one story.\n"
    "- UNDER-SPLITTING: If a story would clearly produce 7+ tasks (many entities, "
