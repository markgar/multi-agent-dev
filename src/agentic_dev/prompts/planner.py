...
PLANNER_INITIAL_PROMPT = (
    ...
    "TEST FRAMEWORK SETUP: The scaffolding story (story 1) MUST include test framework "
    "installation and configuration for every tech layer in the project. For example: "
    "pytest for Python, vitest + testing-library for React/Vite, jest for Node.js, "
    "xUnit/NUnit for .NET. The test runner must be installed, a test script/command "
    "must work (e.g. `npm test`, `pytest`, `dotnet test`), and one trivial placeholder "
    "test must pass. This is critical because the tester agent runs after every milestone "
    "and can only write test files — it cannot install frameworks or modify project "
    "configuration files. If the test framework is not set up in scaffolding, the tester "
...
PLANNER_PROMPT = (
    ...
    "TEST FRAMEWORK SETUP: If this is the scaffolding story (story 1), the milestone "
    "MUST include tasks to install and configure the test framework for every tech "
    "layer in the project. For example: pytest for Python, vitest + testing-library "
    "for React/Vite, jest for Node.js, xUnit/NUnit for .NET. Include a task to add "
    "the test runner, configure a test script/command (e.g. `npm test`, `pytest`, "
    "`dotnet test`), and write one trivial placeholder test that passes. A separate "
    "tester agent runs after every milestone and can only write test files — it cannot "
    "install frameworks or modify project configuration. If the test framework is not "
    "set up in scaffolding, the tester cannot function.\n\n"
