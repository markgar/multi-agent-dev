...
PLANNER_INITIAL_PROMPT = (
    ...
    "BACKLOG.md and files in the `milestones/` directory.\n\n"
    ...
    "stories (e.g. 'Members — Backend' then 'Members — Frontend'). CRITICAL: when "
    "you split a feature into backend and frontend stories, BOTH stories MUST appear "
    "in the backlog. A backend-only story without a corresponding frontend story is "
    "incomplete — the requirement is not covered until the UI exists. Never create a "
    "backend story without its frontend counterpart.\n"
    "- DEFAULT: keep backend + frontend together as ONE story whenever feasible (6-7 "
    "tasks total). Only split when either half alone would need 5+ tasks.\n\n"
    ...
    "Create the directory `milestones/` if it doesn't exist. Write the first "
    "milestone as `milestones/<slug>.md` where `<slug>` is a kebab-case slug "
    "derived from the milestone name (e.g., `milestone-01-project-scaffolding.md`). "
    "The file must start with `## Milestone: <name>`, followed by the "
    "`> **Validates:**` block, followed by task checkboxes. "
    "Do NOT create TASKS.md.\n\n"
    "ONE STORY PER MILESTONE: The first milestone covers ONLY story #1. Do NOT "
    "combine multiple stories into a single milestone. Each story gets its own "
    "milestone — the milestone planner will expand subsequent stories one at a time "
    "between build cycles.\n\n"
    ...
    "the builder should study to understand the project's patterns. The builder uses "
    "these files to learn the conventions and then applies them — this avoids reading "
    "the entire codebase. Pick files from a COMPLETED feature that best represent "
    "the patterns the new milestone should follow.\n"
    "Example:\n"
    "```\n"
    ...
    "MembersPage.tsx (list page with search), MemberEditPage.tsx (form page with "
    "validation). Also read Program.cs for DI registration pattern.\n"
    "```\n"
    "RULES for reference files:\n"
    "- List ONE file per architectural layer (entity, repository, service, controller, "
    "page) — not all files of that type.\n"
    "- Pick files from the feature that is MOST SIMILAR to the new milestone's work.\n"
    "- Include shared infrastructure files the builder must modify (e.g. Program.cs "
    "for DI, AppDbContext.cs for DbSet registration, App.tsx for routing).\n"
    "- For the first feature milestone (no prior features exist), list only "
    "infrastructure files (Program.cs, project files, copilot-instructions.md).\n"
    "- For frontend milestones that follow an existing pattern, list ONE page component "
    "and ONE form component as exemplars — not every page in the app.\n\n"
    "TASK SIZING RULES:\n"
    ...
    "to BACKLOG.md or test tasks to milestones — they will never be planned into "
...
PLANNER_COMPLETENESS_PROMPT = (
    ...
    "story.\n\n"
    "FRONTEND COVERAGE CHECK: If REQUIREMENTS.md describes UI elements (pages, forms, "
    "navigation, admin views, member views, responsive layouts), verify that stories "
    "exist to BUILD those pages — not just the backend APIs they call. A backend API "
    "story does NOT satisfy a UI requirement. Look for stories that explicitly mention "
    "pages, forms, views, or frontend components. If a feature has a backend story "
    "but no corresponding frontend story, add the frontend story.\n\n"
    "STEP 3 — FIX GAPS:\n"
...
BACKLOG_QUALITY_PROMPT = (
    ...
    "story quality in BACKLOG.md and the first milestone in `milestones/`. You must NOT "
    "write any application code or modify any source files other than BACKLOG.md "
    "and files in `milestones/`.\n\n"
    "STEP 1 — READ:\n"
    "Read BACKLOG.md, the milestone files in `milestones/`, REQUIREMENTS.md, and SPEC.md.\n\n"
    "STEP 2 — EVALUATE each criterion below. For any failure, fix BACKLOG.md and/or "
    "the milestone files directly.\n\n"
    ...
    "CRITERION C5b — FRONTEND COVERAGE (FULLSTACK PROJECTS ONLY):\n"
    "If REQUIREMENTS.md describes UI elements (pages, forms, navigation, admin views, "
    "responsive layouts, member-facing views), verify that stories exist to BUILD those "
    "pages — not just the backend APIs they consume. For each feature that has a "
    "backend-only story (name contains 'Backend' or 'API' without mentioning pages), "
    "check whether a corresponding frontend story exists or whether the story "
    "description includes frontend tasks. If a feature has backend coverage but "
    "REQUIREMENTS.md describes UI for that feature and no frontend story exists, "
    "add the frontend story. Skip this check for API-only projects with no UI "
    "requirements.\n\n"
    "CRITERION C6 — FIRST MILESTONE TASK QUALITY:\n"
    "For each task in the first milestone file in `milestones/`: (a) does it include "
    "enough detail (file paths, config keys, field names) for the builder? (b) does it "
    "describe one logical change (not 'and'/'with' connecting distinct work)? (c) does "
    "it create 3 or fewer files/endpoints/entities? If 2+ tasks fail, fix them.\n\n"
    "CRITERION C7 — VALIDATES BLOCK QUALITY:\n"
    "The > **Validates:** block must contain at least 3 specific, testable assertions "
    "referencing concrete commands, endpoint paths with HTTP methods and expected "
    "status codes, page URLs, or CLI commands. If the block says something vague "
    "like 'the application should work', rewrite it with specifics.\n\n"
    "STEP 3 — COMMIT:\n"
    "If you made any changes, commit BACKLOG.md with message '[planner] Backlog "
    "quality fixes' and milestone files with message '[planner] Milestone quality fixes'. "
...
BACKLOG_ORDERING_PROMPT = (
    ...
    "followed by 'Members — Frontend'), not separated into backend-only and "
    "frontend-only blocks. ANTI-PATTERN: if all backend stories are clustered at "
    "the beginning and all frontend stories are pushed to the end, reorder so each "
    "feature's frontend story immediately follows its backend story. Users should "
    "see working UI progressively, not get a wall of API-only milestones followed "
    "by a wall of frontend milestones.\n"
