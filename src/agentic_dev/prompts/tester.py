...
TESTER_MILESTONE_PROMPT = (
    ...
    "FILE RESTRICTIONS: You may ONLY create or modify files in test directories "
    "(e.g. tests/, __tests__/, *.test.ts, *.spec.ts, *.Tests/ projects). Do NOT "
    "modify source code, configuration files, package.json, .csproj, requirements.txt, "
    "or any non-test file. If the test framework is not installed, a test script is "
    "missing, or you need a dependency that is not available, file a bug: "
    "`gh issue create --title '[bug] Test infrastructure missing: <what is needed>' "
    "--body '<details>' --label bug,{milestone_label}` and move on. Do not attempt "
    "to install frameworks or modify project configuration yourself — a builder will "
    "pick up the bug and fix it. "
    ...
    "Commit all new tests, run git pull --rebase, and push. "
