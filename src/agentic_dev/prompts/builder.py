...
BUILDER_PROMPT = (
    ...
    "`bug-`/`fixed-` prefix). These are open bugs. "
    "{partition_filter}"
    "If there are any open bugs assigned to you, fix ALL "
    ...
    "which commit fixed it, commit with a meaningful message, and push. "
    ...
    "the `finding-`/`resolved-` prefix). These are open findings. "
    "Apply the same digit filter to findings. "
    "Address them one "
