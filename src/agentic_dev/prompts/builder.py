...
BUILDER_FIX_ONLY_PROMPT = (
    ...
    "Your job is to fix the specific bugs and review findings listed below. "
    "There is no milestone to work on right now — focus entirely on fixing "
    "these issues. Do NOT run `gh issue list` — the issues to fix are already "
    "provided.\n\n"
    "ISSUES TO FIX:\n"
    "{issue_list}\n\n"
    "For each issue above: read its details with "
    "`gh issue view <number> --json title,body`, fix the problem, close it with "
    "`gh issue close <number> --comment 'Fixed in <commit-sha>: <description>'`, "
    "commit your code fix with a meaningful message, and push.\n\n"
    "After any refactoring, check for dead code left behind and remove it. "
    "When all listed issues are fixed, you are done for this session."
)
