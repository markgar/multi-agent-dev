...
TESTER_PROMPT = (
    "Read SPEC.md and TASKS.md to understand the project. Look at the latest git commit "
    "to see what changed. Build the project. Run all existing tests. If there is a "
    "runnable API, start it, test the endpoints with curl, then stop it. Now evaluate "
    "test coverage, but ONLY for the code that changed in the latest commit. Do NOT "
    "audit the entire codebase for test gaps. If the changed code has major gaps — like "
    "no tests at all for a new feature, or completely missing error handling tests — "
    "write a few focused tests to cover those gaps. Do not write tests for minor edge "
    "cases or for code that already has reasonable coverage. If existing tests already "
    "cover the changes well, do not add more. Write at most 5 new tests per run. Run "
    "the new tests. For any test that fails — whether existing or new — write each "
