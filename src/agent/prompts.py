...
PLANNER_PROMPT = (
    ...
    "message '[planner] Update spec for new requirements', run git pull --rebase, and push. Then "
    "proceed to planning.\n\n"
    ...
    "TASK AND MILESTONE SIZING RULES (follow these strictly): "
    "Each task is completed by an AI coding agent in a single session with limited "
    "context window. Tasks MUST be small enough that the agent can understand what to "
    "do, write the code, and commit — all without needing to summarize or compress "
    "context. A good task is ONE file or ONE small group of tightly related files. "
    "TASK SIZE RULES: "
    ...
    "  GOOD: 'Create solution and Stretto.Domain project' then 'Add Stretto.Application "
    "  and Stretto.Infrastructure projects with references' then 'Add Stretto.Api project "
    "  and configure project references' "
    ...
    "- BAD: 'Add .editorconfig and configure Program.cs with health endpoint and Swagger' "
    "  GOOD: 'Create Program.cs with minimal web server and health endpoint' then "
    ...
    "- If a feature area needs more than 5 tasks after proper splitting, create "
    "sequential milestones (e.g. 'Project scaffolding: backend' then 'Project "
    "scaffolding: frontend'). "
    "- More milestones with smaller tasks is ALWAYS better than fewer milestones "
    "with packed tasks. Never stuff multiple concerns into one task just to stay "
    "under the 5-task cap. "
    "- If several truly small related pieces fit under 5 tasks, combine them into "
    "one milestone. "
    ...
    "message '[planner] Update task plan', run git pull --rebase, and push. If no changes are "
    "needed, do nothing."
)
...
COPILOT_INSTRUCTIONS_PROMPT = (
    ...
    "with a SHORT role description (what the file IS, not how it's implemented). "
    "Example: 'src/server.js — HTTP server entry point' NOT "
    "'src/server.js — Express server that uses middleware X and calls function Y with "
    "a Map<string, User> for storage'. These are predictions — they will be updated "
    "as code is written.\n\n"
    "For 'Architecture': describe how the system works at a high level — major "
    "components, how they communicate, data flow. Keep it to 3-5 sentences. Do NOT "
    "specify implementation details like class names, data structures, or specific "
    "library APIs. Example: 'The backend exposes a REST API. The frontend is a single-page "
    "app that communicates with the backend via fetch. The backend serves the frontend's "
    "production build as static files.' NOT 'The backend uses a List<Note> with an "
    "auto-incrementing integer counter stored in a static variable.'\n\n"
    "For 'Conventions': list coding STYLE and PATTERN conventions — things like API "
    "response format, error handling approach, or naming patterns. Do NOT include "
    "implementation instructions like 'put all code in one file', 'use record types', "
    "or 'use an in-memory list'. Conventions describe HOW to write code, not WHAT code "
    "to write. Good examples: 'All API responses use a consistent JSON envelope', "
    "'Use the framework's built-in error handling middleware'. Bad examples: 'Use a "
    "List<T> for storage', 'Define all endpoints in Program.cs', 'Use a record type "
    "for the model'.\n\n"
