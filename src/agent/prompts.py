...
PLANNER_PROMPT = (
    ...
    "so the app can be built and launched (even if it does nothing useful yet). It must "
    "also respond to at least one request — a health check endpoint, a root route, a "
    "--help flag, or similar — so that a container-based validator can confirm it works. "
    "Never "
    ...
    "features are added. "
    "CONTAINERIZATION: Include a task in the first milestone to create a Dockerfile "
    "(and docker-compose.yml if the app needs a database or other services) so the "
    "application can be built and run in a Docker container from the start. A separate "
    "validator agent will build and test the container after each milestone. "
