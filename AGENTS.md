...
1. **Bootstrap** — runs Copilot to create the repo (README.md, SPEC.md, REQUIREMENTS.md, bugs/)
...
**Creates:** README.md, SPEC.md, REQUIREMENTS.md, bugs/, git repo, GitHub remote  
**Writes code:** No
...
**Note:** Before Copilot runs, the CLI pre-creates `builder/REQUIREMENTS.md` containing the prompt or spec-file content verbatim, and creates `builder/bugs/` directory (with a `.gitkeep` file so git tracks it). These are committed with the rest of the bootstrap. In later sessions, `go` may overwrite REQUIREMENTS.md with new requirements before re-planning.
...
**Skips:** Non-milestone merge commits, coordination-only commits (milestone files only, or bugs/ only)  
...
**Trigger:** Polls `logs/milestones.log` every 10 seconds (configurable via `--interval`)  
**Scope:** Three checks per milestone: (A) current milestone validation against its `> **Validates:**` block, (B) requirements coverage by cross-referencing REQUIREMENTS.md and BACKLOG.md against the running app, (C) fixed bug verification by re-testing `bug-*.md` files that have matching `fixed-*.md`  
**Reads:** SPEC.md, DEPLOY.md, REQUIREMENTS.md, BACKLOG.md, `milestones/` (current only), `bugs/` (for fixed bug verification)  
...
- Agents never edit or delete existing files in `bugs/` — they only create new files. This eliminates merge conflicts on that directory.
...
- All agents run `git pull --rebase` before pushing to avoid merge conflicts. Since `bugs/` is an append-only directory (no file is ever edited), concurrent new-file creations never conflict.
...
3. **Check work lists:** The builder pulls latest and scans `bugs/` for open bugs (bug-* without fixed-*), GitHub Issues for open findings (`gh issue list --label finding --state open`), and milestone files for unchecked items.
